...
7. Run a generation:
   ...
   ```text
   python <skill-directory>/scripts/generate.py --task-id "task-<fresh-unique-id>" --prompt "<prompt>" --size 4K --aspect-ratio 2:3 --quality high
   ```

   For an edit with an original image:

   ```text
   python <skill-directory>/scripts/generate.py --task-id "task-<fresh-unique-id>" --prompt "<edit instructions>" --image "<exact prior result path>" --size 4K --quality high
   ```
