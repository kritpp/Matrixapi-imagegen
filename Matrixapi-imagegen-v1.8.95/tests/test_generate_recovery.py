...
class AsyncResultRecoveryTests(unittest.TestCase):
    ...
    def test_model_discovery_reads_official_ids_visible_to_the_key(self) -> None:
        response = io.BytesIO(
            generate.json.dumps(
                {
                    "data": [
        ...
        with mock.patch.object(
            ...
            available = generate.discover_available_models(
                "https://matrixapii.com/v1/models", "secret", timeout=7
            )

        self.assertEqual(
            available, {"gemini-3-pro-image-preview", "gpt-image-2"}
        )
        request = urlopen.call_args.args[0]
        self.assertEqual(request.full_url, "https://matrixapii.com/v1/models")
    ...
    def test_default_model_adapts_to_key_routes_without_changing_explicit_choices(self) -> None:
        ...
        self.assertEqual(
            generate.select_model_for_available_routes(
                generate.DEFAULT_MODEL, None, explicit_selection=False
            ),
            generate.DEFAULT_MODEL,
        )
    ...
    def test_main_keeps_story_in_progress_after_unknown_submit_outcome(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(generate.sys, "argv", argv), mock.patch.object(
                ...
                generate, "discover_available_models", return_value=None
            ), mock.patch.object(
