...
The pinned relay must implement `GET /v1/models` for API-key-scoped route discovery, `POST /v1/images/generations` for JSON generation, URL-reference editing, async tasks, and SSE, and `POST /v1/images/edits` with multipart `image`/optional `mask` fields for local-file compatibility. For GPT Image 2 models, the pinned relay can convert local image parts to short-lived reusable URLs before calling the model JSON endpoint. Local mask editing is disabled by default for the pinned GPT Image 2 channel because its documented interface does not guarantee `mask`. Only after the relay confirms support may `IMAGEGEN_MASK_SUPPORT=1` be set for a request. Responses may return either `data[].url` or `data[].b64_json`. The supported customer-facing model names are `gpt-image-2.5-flare`, `gpt-image-2.5-sunburst`, `gpt-image-2`, and `gemini-3-pro-image-preview`; the default model and `IMAGEGEN_MODEL` remain `gpt-image-2.5-flare`. Internal relay aliases are implementation details and must not be exposed.
