...
3. Choose the requested size. All supported upstreams use `1K` as the lowest tier; use `1K` by default unless the customer explicitly requests `2K`/`4K`. Do not run or narrate a separate size or aspect-ratio preflight. For every local edit, pass `--aspect-ratio auto`; the script preserves the first input image's native geometry while selecting the lowest valid tier before the only paid submission. Never replace a source ratio with the nearest enum, never force `1:1`, `3:2`, or `2:3` for an edit, and never turn an edit into a square `2048x2048` request. If the relay rejects native geometry, report that upstream error once; do not silently change the ratio and submit a second image request. If billing or final state is unknown, query/idempotency-guard the original task and do not submit again. Do not crop, stretch, recomposite, or locally redraw the image.
   If the user asks for an arbitrary final pixel size, do not add `--output-size` automatically to an edit. Use `--output-size WIDTHxHEIGHT`, cropping, or format conversion only when the customer explicitly requests local post-processing. For an edit, return the upstream image at its native returned geometry.
