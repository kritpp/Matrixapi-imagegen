...
3. Choose the requested size. All supported upstreams use `1K` as the lowest tier; use `1K` by default unless the customer explicitly requests `2K`/`4K`. Do not run or narrate a separate size preflight. For a local edit, pass `--aspect-ratio auto`; the script silently selects the lowest locally valid tier before the only paid submission. If a wide/tall source falls below the minimum pixel area at `1K`, the same command selects ratio-preserving `2K` before calling the API. Never submit a failing 1K request followed by a second 2K request, never force `1:1`, `3:2`, or `2:3`, and never turn an edit into a square `2048x2048` request. If billing or final state is unknown, query/idempotency-guard the original task and do not submit again. Do not crop, stretch, recomposite, or locally redraw the image.
