...
7. Run a generation:
   ...
   Generate a fresh `--task-id` before every command and never reuse it. The script separately computes a stable request fingerprint from the current Codex thread, prompt, model, mode, size, quality, count, ordered reference URLs, ordered local reference-image content, mask, and output options. If Codex accidentally executes the same paid request again with a new task id, the script returns the existing successful files through a new current-task JSON without calling the API again. The initial submission may be attempted at most three times, using the same request fingerprint and `Idempotency-Key`, only for HTTP 502/503/504 or a connection failure that is clearly before submission. It never resubmits after a returned image, task id, polling/response timeout, unknown billing state, content refusal, ordinary 400, or local validation error. After the third safe transient failure, the identical request is locked until the user explicitly approves `--force-new`. Never add `--force-new` as an automatic recovery step. Reference images are scoped to the current request, not the whole conversation. Do not pass every image mentioned earlier. For a new page or variant, use the explicitly requested current-turn references and, only when the request is an edit, the exact latest generated output from this conversation; do not append older generated 4K outputs to the next request. For a repeated edit, replace the previous input set with the exact latest output unless the user explicitly asks to keep another reference.
...
8. Apply local deterministic post-processing only when the user requests a final size, crop, output format, compression, or derived variants. The local stage may resize, crop, fit to a canvas, convert PNG/JPEG/WebP/AVIF, compress, and write a manifest; it must not install or invoke a local ML model. Use `--output-size`, `--fit cover|contain|fill|inside|outside`, `--position`, `--crop`, `--output-format`, `--output-quality`, and `--output-background` as needed. Preserve the unmodified model result in `original_files` for future edits, and use `processed_files` for the requested deliverable. For an existing image with no API request, use `--process-only --image <path>` with the same local options.
   For an ordinary request such as “把上一张图里的青春同频改成牵手情缘”, pass the exact preceding result as `--image`, add `--replace-text "青春同频" --new-text "牵手情缘"`, and keep any artistic-style instruction in `--prompt`. This remains a model edit: the script adds exact-character constraints and locks every unrelated image element and all other text, without an extra planning/model call.
   When the user explicitly says “精准修改文字”, “文字必须完全正确”, “只改文字，图片完全不变”, “不要重新生成图片”, or “本地修改文字”, use local precise-text mode instead: `--precise-text --image <exact prior path> --new-text "..." --text-box x,y,width,height`. Select the known text region from the visible image; do not run OCR or scan directories. Optional `--font-path`, `--font-size`, `--text-color`, and `--output-background` tune the deterministic replacement. It preserves the source and writes a separate PNG without calling or charging the image API. A flattened image with a complex artistic title may require a supplied clean background/source file for truly lossless restoration; report that limitation rather than silently invoking the model.
9. Parse only the JSON written to stdout by the current command. Accept it only when `ok` is true, its `task_id` exactly equals the command's `--task-id`, `result_match.task_id` matches it, and `completed_at_ms` is not earlier than `request_started_at_ms`. A response with `idempotency_reused: true` is valid when these current-task checks pass; it means the script safely reused the exact same recent request instead of charging again. Use its `preview_files` immediately; do not open or scan the output directory, search for a newer file, reread the result JSON with another shell command, inspect dimensions, call `view_image`, or run another verification command. Do not keep narrating, waiting, or issue another model/tool call after successful JSON is received. The script atomically writes that same task-scoped JSON and, on Windows, schedules it to become hidden after stdout has been delivered; this background hide does not delay command completion. For each output, render both the inline preview and a clickable original-image link, using the normalized absolute paths from `preview_files` and `download_files`:
