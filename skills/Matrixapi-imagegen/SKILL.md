...
7. Run a generation:
   ...
   For URL-based reference editing, repeat `--reference-url "https://..."` up to 16 times instead of using `--image`. For masked local editing, add `--mask "<mask path>"` only after the model channel has confirmed mask support. On the pinned relay, use ordinary reference editing instead; describe the target area and blending constraints in the prompt. Keep `n` at 1 unless the user explicitly requests variants; the maximum is 4. For `gpt-image-2-pro` edits, preserve the source geometry: leave `--aspect-ratio auto`, omit every local post-processing option, and return the model's original file. Never add `--allow-pro-postprocess` unless the user explicitly requested a final local size, crop, or format. The script rejects accidental Pro post-processing before upload, so a guessed size cannot cause a charge or a second resize/crop.
