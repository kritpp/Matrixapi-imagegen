...
4. Choose the request mode automatically: no `--image` uses `/v1/images/generations`; one or more `--image` files uses multipart `/v1/images/edits`; `--mask` is optional for edit/inpaint requests. Repeat `--image` for multiple reference images, up to seven. Send the requested size unchanged for edits, including 1K, 2K, and 4K sizes; the provider decides whether that size is supported and may return an error. The original input file is never overwritten.
...
6. Parse the JSON written to stdout. For each output, use the matching item in `image_info` to report the actual saved image dimensions, file format, and resolution label before rendering its preview. The script determines these values from the output file itself, not from the requested size. Render both the inline preview and a clickable original-image link, using the normalized absolute paths from `preview_files` and `download_files`:
   ...
   For every generation or edit request, require `ok: true`, a matching `request_id`, a non-empty `preview_files` entry, and an existing file at each exact returned path before rendering. After the image has been downloaded and its dimensions validated, the script immediately writes the current-task ready sidecar `.ready-<task-id>.json`; it then writes `.result-<task-id>.json` and completion marker `.completed-<task-id>.json`. On Windows all three JSON sidecars are marked hidden in Explorer while remaining readable at the same paths. If the command wrapper says it is still running, poll this task's ready sidecar first and render immediately when it is valid; do not wait for the final marker and do not submit another request. Never scan, sort, or inspect the shared generated-images directory, choose the newest or first image, or use any path from an earlier request, conversation, or Codex installation. If the task has a real API/script failure, preserve the existing retry policy and use only the returned JSON from the retry that actually succeeds. A ready sidecar also reserves the task ID, so rerunning it requires explicit retry permission. These rules apply equally to new images, edits, redraws, and second-pass modifications.
