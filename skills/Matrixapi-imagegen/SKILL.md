...
2. Turn the user's request into a complete image prompt. Preserve requested subject, composition, style, lighting, colors, text, and constraints. For comic or storyboard images, default to no visible text, dialogue, speech bubbles, captions, or lettering; only include visible text when the user explicitly requests it. Do not invent identifying details. When the user supplies local images, use only the exact attachment paths exposed by the current user message, in attachment order, and pass every one as a separate `--image`. If the user explicitly says to modify the image just generated, the image above, or the previous result, reuse only the exact `preview_file`/`preview_files` path from the immediately preceding successful result JSON in this same Codex conversation. Never scan a temp, clipboard, Downloads, workspace, or generated-images directory to discover or guess input images, and never substitute an image from another conversation or an unrelated task. Pass a supplied mask as `--mask`.
3. Choose the requested size, or use `1024x1024` when none is given. Common larger sizes are `2048x2048` for square 2K, `2048x1152` for landscape 2K, `3840x2160` for landscape 4K, and `2160x3840` for portrait 4K. A portrait request with no explicit ratio defaults to `2:3`; preserve any ratio explicitly requested by the customer. Both edges must be multiples of 16, the longest edge must not exceed 3840 pixels or three times the shorter edge, and total pixels must be between 655,360 and 14,745,600. Native GPT Image 2 4K edits are preserved.
...
5. Run a generation with one unique task ID for the whole user request:
   ...
   For masked local editing, add `--mask "<mask path>"`. Keep `n` at 1 unless the user explicitly requests a different total output count. For a large local GPT Image 2 edit (6 or more references or at least 48 MiB), the script automatically submits the first frame as one asynchronous task and polls its status; it never splits the original reference set into separate first-frame requests.

   For a continuous story, comic, or storyboard, add `--sequence` and pass the user's requested page count through `--n`; do not use or invent `--output-prompt`:

   ```text
   python <skill-directory>/scripts/generate.py --request-id "<task-id>" --prompt "<complete story brief>" --expected-images 16 --image "<reference-1>" ... --image "<reference-16>" --sequence --n 3
   ```

   This is one local coordinator process, not one upstream request that asks for several pages at once. It makes distinct upstream requests in order. Request 1 receives the complete story brief plus every original reference and generates only page 1. After page 1 is saved and emitted, request 2 receives the same complete brief constrained to page 2 plus only the saved page 1 file. Request 3 similarly receives only saved page 2, and later pages follow the same rule. Never resend the original references after page 1. Never send page 2 before page 1 is saved. A later failure stops all remaining pages and never resubmits an already billed page. Generic non-story requests for several independent variants use `--n` without `--sequence` and do not chain outputs.

   Run the coordinator script exactly once for the user task so it can preserve page order, billing state, and the exact saved predecessor without directory scans. Do not describe this to the user as “one API request for all pages”; say that page 1 is starting. If the command tool reports that the
...
The installed release is Matrixapi-imagegen 1.8.15. The bundled installer and updater
