...
4. Choose quality from `auto`, `low`, `medium`, `high`, and `max`. Leave quality at the default `auto` unless the customer explicitly requests another level: the dedicated MatrixAI GPT Image 2 route uses `high`, and its 2.5 route upgrades `auto` to `max`. Use explicit `max` only for a GPT Image 2.5 model. If a 2.5 route is unavailable before dispatch and the guarded GPT Image 2 fallback applies, submit that fallback once with `high` and report the actual model and quality. Map "草稿/快速预览" to `low`, "标准" to `medium`, and an explicit `high` request to `high`; do not infer `high` from general requests for detail. Other models and image channels keep their existing behavior. The model price is per request; quality changes latency/detail rather than the configured per-request price.
5. Choose the request mode automatically: no reference image uses `/v1/images/generations`; local `--image` files use the relay's multipart `/v1/images/edits`; public `--reference-url` values use the GPT Image JSON `images` array on `/v1/images/generations`. On the pinned relay, local GPT Image 2 files are staged as short-lived reusable HTTPS URLs and forwarded through the documented JSON `images` flow when the model service does not accept inbound multipart media. The URL remains available during async validation and processing, then expires automatically. Up to 16 references are accepted for the pinned GPT Image 2 model. Do not mix local files and URL references in one request. For this relay, local `mask` is disabled by default for the GPT Image models; the script rejects it locally before upload so a known-unsupported request cannot fail after billing. Do not treat a mask as an ordinary reference image. The VPS identifies its selected route internally; for its Yaliai routes, it packs local multipart references only when the six-image/12 MiB-per-file/30 MiB-total limits require it and sends the resulting <=6 uncropped grids in one request. Other providers remain unchanged.
...
6. The supported customer-facing model names are `gpt-image-2` (default), `gpt-image-2.5-flare`, `gpt-image-2.5-sunburst`, and `gemini-3-pro-image-preview`. The relay's internal aliases are never shown to the customer. Resolve the model independently from the current user turn; never inherit the previous generation or edit model. When the customer does not request a model, omit `--model` from the command even though the configured default is known; never turn the default into an explicit model argument. This omission makes the script perform one read-only `/v1/models` request using the customer's API Key before the paid image request. Keep the configured model when that Key exposes it; otherwise prefer `gpt-image-2`, then `gpt-image-2.5-flare`, `gpt-image-2.5-sunburst`, and finally `gemini-3-pro-image-preview` for a Gemini-only route. A discovery failure preserves the configured model, and normal route discovery never changes `IMAGEGEN_MODEL` or sends an image request. `模型-f2.5` selects `gpt-image-2.5-flare`, while `模型-s2.5` selects `gpt-image-2.5-sunburst`; either marker may appear anywhere in the current prompt and wins over an accidentally supplied default `--model`. Preserve the marker in `--prompt` so the script removes it before sending the image description upstream. Different model markers in one request are a local error and must not be submitted. The older `模型-2` marker remains accepted but is unnecessary because `gpt-image-2` is now the default. Pass `--model` only when the customer explicitly names a full official model ID. Do not add a Gemini prompt suffix. If a selected 2.5 model is rejected before dispatch with an explicit model/channel-unavailable routing error, the script makes one guarded fallback request using `gpt-image-2` so older groups can continue. Generic 502/503 responses, timeouts, or any response that may have reached the upstream are not retried. The terminal JSON reports `model` and `actual_model` as the official model actually selected by route discovery, with `model_fallback` remaining null for discovery; `model_fallback` is populated only for the guarded post-selection legacy fallback. Local GPT Image 2 edits on the pinned relay preserve the requested native size through temporary URL conversion. Other relays may require a public reference URL or an enabled multipart capability. Set `IMAGEGEN_LEGACY_EDIT_RESIZE=1` only when an older relay rejects high-resolution edits; this intentionally changes the working size and is reported in the JSON result. The JSON result includes `aspect_ratio_source` (`input_image`, `output_size`, `user`, or `model_default`) so the selected ratio is visible.
7. Run a generation:

   ```text
   python <skill-directory>/scripts/generate.py --task-id "task-<fresh-unique-id>" --prompt "<prompt>" --size 4K --aspect-ratio 2:3
   ```

   For an edit with an original image:

   ```text
   python <skill-directory>/scripts/generate.py --task-id "task-<fresh-unique-id>" --prompt "<edit instructions>" --image "<exact prior result path>" --size 4K
   ```
   ...
   **New-conversation isolation (hard rule):** a new conversation or a new
   ...
   ```text
   python <skill-directory>/scripts/story_runner.py --task-id "task-<fresh-unique-id>" --story-pages 3 --prompt "<complete story request>" --image "<reference-1>" --image "<reference-2>" --size 4K
   ```
...
The pinned relay must implement `GET /v1/models` for API-key-scoped route discovery, `POST /v1/images/generations` for JSON generation, URL-reference editing, async tasks, and SSE, and `POST /v1/images/edits` with multipart `image`/optional `mask` fields for local-file compatibility. For GPT Image 2 models, the pinned relay can convert local image parts to short-lived reusable URLs before calling the model JSON endpoint. Local mask editing is disabled by default for the pinned GPT Image 2 channel because its documented interface does not guarantee `mask`. Only after the relay confirms support may `IMAGEGEN_MASK_SUPPORT=1` be set for a request. Responses may return either `data[].url` or `data[].b64_json`. The supported customer-facing model names are `gpt-image-2`, `gpt-image-2.5-flare`, `gpt-image-2.5-sunburst`, and `gemini-3-pro-image-preview`; the default model and fresh-install `IMAGEGEN_MODEL` are `gpt-image-2`. Internal relay aliases are implementation details and must not be exposed.
...
required Skill files before replacing anything. It compares versions before
download/replacement and never overwrites an equal or newer installed version
with an older archive. It holds an update lock and keeps
...
touch generated images or `IMAGEGEN_API_KEY`, and it never calls the image
generation API. During the v1.8.96 transition, the configuration check migrates
only the former installer default `IMAGEGEN_MODEL=gpt-image-2.5-flare` to the new
default `gpt-image-2`; every other custom model value is preserved.
