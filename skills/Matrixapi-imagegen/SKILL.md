...
6. Parse the JSON written to stdout. For each output, use the matching item in `image_info` to report the actual saved image dimensions, file format, and resolution label before rendering its preview. The script determines these values from the output file itself, not from the requested size. Render both the inline preview and a clickable original-image link, using the normalized absolute paths from `preview_files` and `download_files`:
   ...
   For every generation or edit request, require `ok: true`, a matching `request_id`, a non-empty `preview_files` entry, and an existing file at each exact returned path before rendering. The script validates the image and dimensions, atomically writes one hidden ready sidecar `.ready-<task-id>.json`, prints the same success JSON, and exits. Treat that success JSON (or a valid matching ready sidecar only when stdout is unavailable) as the completion signal: render immediately and stop. Do not run any directory scan, sort, process check, dimension recheck, result-marker wait, or extra command after a valid result. Never scan, sort, or inspect the shared generated-images directory, choose the newest or first image, or use any path from an earlier request, conversation, or Codex installation. If the task has a real API/script failure, preserve the existing retry policy and use only the returned JSON from the retry that actually succeeds. A ready sidecar also reserves the task ID, so rerunning it requires explicit retry permission. These rules apply equally to new images, edits, redraws, and second-pass modifications.
